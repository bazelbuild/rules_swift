...
"""Tests for `swift_library.private_deps` in conjunction with `swift.expose_private_deps_as_public` feature."""
...
private_deps_as_public_provider_test = make_provider_test_rule(
    config_settings = {
        "//command_line_option:features": [
            "swift.supports_private_deps",
            "swift.expose_private_deps_as_public",
        ],
    },
)

# Test with enabled `swift.add_target_name_to_output` feature
private_deps_as_public_provider_target_name_test = make_provider_test_rule(
    config_settings = {
        "//command_line_option:features": [
            "swift.supports_private_deps",
            "swift.expose_private_deps_as_public",
...
def private_deps_as_public_test_suite(name, tags = []):
    """Test suite for propagation behavior of `swift_library.private_deps` with \
    the `swift.expose_private_deps_as_public` feature enabled.
    ...
    private_deps_as_public_provider_test(
    ...
    # The client module should propagate its own module and the modules from
    # both `deps` and `private_deps`.
    private_deps_as_public_provider_test(
    ...
    # We should propagate compilation contexts of the private deps that are C++
    # libraries. That means that with `swift.expose_private_deps_as_public` enabled,
    # public deps' as well as private deps' headers will be repropagated by
    # the Swift library.
    private_deps_as_public_provider_test(
    ...
    # Likewise, we should repropagate the C++ private deps' module maps.
    private_deps_as_public_provider_test(
    ...
    private_deps_as_public_provider_target_name_test(
    ...
    # deps. All libraries should be propagated.
    private_deps_as_public_provider_test(
