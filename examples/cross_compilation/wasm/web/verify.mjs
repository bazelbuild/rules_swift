...
const wasmPath =
    process.argv[2] ?? 'bazel-bin/examples/cross_compilation/wasm/Reactor.wasm';
...
const wasi = {
  ...
  fd_seek: (fd, off, whence, newOff) => {
    dv().setUint32(newOff, 0, true);
    return SUCCESS;
  },
  fd_write: (fd, iovs, n, nwritten) => {
    let written = 0;
    for (let i = 0; i < n; i++)
      written += dv().getUint32(iovs + i * 8 + 4, true);
...
const greeting =
    new TextDecoder().decode(new Uint8Array(memory.buffer, ptr, written));
