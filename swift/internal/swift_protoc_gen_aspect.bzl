...
def _declare_pbswift_files(target, actions, proto_srcs):
  ...
  """
  return [
      actions.declare_file(_pbswift_file_path(target, f)) for f in proto_srcs
  ]
...
def _register_pbswift_generate_action(
  ...
  protoc_args.add(protoc_gen_swift_executable,
...
def _swift_protoc_gen_aspect_impl(target, aspect_ctx):
  ...
  transitive_module_mapping_file = _register_module_mapping_write_action(
      target, aspect_ctx.actions, minimal_module_mappings)

  if direct_srcs:
    # Generate the Swift sources from the .proto files.
    pbswift_files = _register_pbswift_generate_action(
        target, aspect_ctx.actions, direct_srcs, transitive_descriptor_sets,
        transitive_module_mapping_file, aspect_ctx.executable._mkdir_and_run,
        aspect_ctx.executable._protoc, aspect_ctx.executable._protoc_gen_swift)
  ...
  providers.append(_build_swift_proto_info_provider(
      pbswift_files, minimal_module_mappings, deps))
