...
_ThingInfo = provider(
    fields = {
        "pcm_file": "The precompiled module file for this clang module, if it was precompiled.",
    },
)
...
def _apple_sdk_clang_module_impl(ctx):
    ...
    all_pcms = [precompiled_module] if precompiled_module else []
    for dep in deps:
        if _ThingInfo in dep and dep[_ThingInfo].pcm_file:
            all_pcms.extend(dep[_ThingInfo].pcm_file)
