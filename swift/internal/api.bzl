...
def _global_module_cache_path(bin_dir):
    ...
    The global module cache is used if the `swift.use_global_module_cache`
    feature is enabled. Note that the use of this global cache is non-hermetic;
    the cached modules are not declared inputs or outputs and they are not wiped
    between builds. The cache is purely a build performance optimization that
    reduces the need for large dependency graphs to repeatedly parse the same
    ...
    Returns:
        The path to the global module cache.
    """

    # This path explicitly differs from the one used for Objective-C. This is
    # because the Clang invocation for Objective-C does not produce the same
    # hashes as the invocation used internally by Swift's ClangImporter. This
    # means that the cached modules will never actually be shared, and making
    # the paths the same would be at best useless and at worse actively harmful
    # if there ever were to be a collision. We can revisit this decision if it
...
def _swiftc_command_line_and_inputs(
    ...
    Args:
        args: An `Args` object into which the command line arguments will be
            added.
        feature_configuration: A feature configuration obtained from
            `swift_common.configure_features`.
        module_name: The name of the Swift module being compiled. This must be
