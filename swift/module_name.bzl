...
def validate_swift_module_name(module_name, *, feature_configuration = None):
    ...
    non-raw identifier or a properly escaped raw identifier. If the module name
    was generated by an aspect (contains a %), it validates that the aspect ID
    matches the suffix.

    Args:
        module_name: The source name of the module.
        feature_configuration: The feature configuration used to compile.
    """
    valid_format = False
    if module_name.startswith("`") and module_name.endswith("`"):
        valid_format = True
    elif _is_valid_non_raw_identifier(module_name):
        valid_format = True
