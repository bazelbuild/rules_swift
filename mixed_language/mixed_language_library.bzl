...
def mixed_language_library(
    ...
    internal_tags = (["manual"] + tags) if "manual" not in tags else tags

    if not kwargs.pop("generates_header", True):
        fail(
            """\
Mixed language libraries must generate an Objective-C interop header.\
""",
            attr = "generates_header",
        )
    if "copts" in kwargs:
        fail(
            "Use 'clang_copts' and/or 'swift_copts' to set copts.",
            attr = "copts",
        )
    if "defines" in kwargs:
        fail(
            "Use 'clang_defines' and/or 'swift_defines' to set defines.",
            attr = "defines",
        )
    if "implementation_deps" in kwargs:
        fail(
            """\
'implementation_deps' is not supported. Use 'private_deps' to set \
implementation deps.\
""",
...
""",
    ...
    if not module_map:
        ...
        mixed_language_internal_module_map(
            ...
            # duplicated headers being found. Ther is probvably a way to make
    ...
    native.objc_library(
        ...
        hdrs = adjusted_hdrs,
        aspect_hints = [":" + internal_swift_interop_name],
    ...
    swift_library(
        ...
        # This should be the default for `swift_library`, but is a breaking
        # change
    ...
    native.objc_library(
        ...
        # The aspect hint isn't needed here because `_mixed_language_library`
        # will explciitly set the module name. We set it here for
        # rules_xcodeproj or other aspects to be able to assign a sane module
        # name to the target. Since we use the target for the `_headers` target,
        # this has minimal additional overhead.
        aspect_hints = [":" + internal_swift_interop_name],
        copts = clang_copts,
        defines = clang_defines,
        ...
        features = features,
        includes = includes,
        # The `_swift` target is an implementation dep because the
    ...
    # targets will set it via implicit search.
